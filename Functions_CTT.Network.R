...
import.node.health <- function(INFILE, NODE.VERSION, RADIOID, TIMEZONE, START, END, LAT.LOWER, LAT.UPPER, LONG.LOWER, LONG.UPPER) {
  ...
  node_import.names <- names(node_import) <- stringr::str_sub(basename(node_files_date), start = 5, end = 16) # alternativly with base r -  substr(node_files, start = 1, stop = 12)
...
# Define the center of network array based on current data
  centerLng <- mean(node.loc.summary$lng)#min(node.loc.summary$lng) - ((max(node.loc.summary$lng) - min(node.loc.summary$lng))/2)
  centerLat <-mean(node.loc.summary$lat)#min(node.loc.summary$lat) + ((max(node.loc.summary$lat) - min(node.loc.summary$lat))/2)
