...
###EDIT THESE VALUES
infile <- "G:/Shared drives/aplab/research_projects/DE_quail/data/"
outpath <- "G:/Shared drives/aplab/research_projects/DE_quail/data/"

tags <- read.csv(paste0(outpath,"Tags.csv"), header = T, as.is=TRUE, na.strings=c("NA", "")) #uppercase node letters
...
nodes<-nodes %>% filter(!grepl('GPS', comment), deployed=="Y")
...
locations <- weighted_average(freq[1], beep_data, nodes, MAX_NODES=max_nodes, tag_id=tag_id) #all_data[[2]][[1]], 
...
st_write(locs, paste0(outpath,"/locations.shp"), append=F)
